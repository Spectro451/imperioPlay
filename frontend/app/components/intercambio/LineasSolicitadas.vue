...
import type { LineaSolicitada } from '~/composables/useIntercambioEnCurso'
...
<template>
  <div class="bg-bg-card border border-border rounded-lg overflow-hidden">
    <div class="px-4 py-3 border-b border-border bg-bg-hard flex items-center justify-between">
      <p class="text-xs uppercase tracking-widest text-muted font-bold">Se lleva el cliente</p>
      <p class="text-[10px] text-muted">Juegos del inventario</p>
    </div>

    <div v-if="!lineas.length" class="p-8 text-center text-muted text-sm">
      Buscá juegos por SKU o nombre para agregarlos.
    </div>

    <ul v-else class="divide-y divide-border">
      <li
        v-for="linea in lineas"
        :key="`juego-${linea.variante.id}`"
        ...
        <div class="min-w-0 flex-1 flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            ...
            <p class="text-xs text-muted capitalize mt-0.5">
              {{ linea.variante.plataforma }} · {{ linea.variante.estado }}
            </p>
            <p v-if="linea.variante.sku" class="text-xs text-muted mt-0.5 font-mono">
              SKU: {{ linea.variante.sku }}
            </p>
            <p class="text-[10px] text-muted mt-0.5">
              ${{ linea.variante.precio_final.toLocaleString('es-AR') }} · Stock: {{ linea.variante.stock }}
            </p>
          </div>
          <button
        ...
        <div class="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
          ...
          <div class="shrink-0 sm:w-24 sm:text-right">
            ...
            <p class="text-sm font-bold text-acento-1">
              {{ linea.variante.tier }} × {{ linea.cantidad }}
            </p>
          </div>
