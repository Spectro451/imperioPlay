...
const agotado = computed(() => props.item.stock === 0)
...
<template>
  <NuxtLink
    :to="link"
    :class="[
      'group bg-bg-card rounded-lg overflow-hidden flex flex-col transition-transform duration-200',
      agotado ? 'hover:scale-[1.01]' : 'hover:scale-[1.02]',
  ...
  >
    ...
    >
      ...
      <span
        ...
        class="absolute top-2 right-12 bg-bg-hard text-acento-1 text-xs font-bold px-2 py-0.5 rounded"
      >
        Últimas {{ item.stock }}
      </span>

      <div class="absolute top-2 right-2 z-10">
        <BotonWishlist :tipo="item.tipo" :variant-id="item.id" />
      </div>
      ...
      >
        <div class="w-[200%] -rotate-45 bg-neutral-800 text-white text-center py-1.5 text-sm font-black tracking-[0.35em] shadow-lg">
          AGOTADO
        </div>
      </div>
    </div>

    <div :class="['p-3 flex flex-col gap-1 flex-1', agotado && 'opacity-60']">
      ...
      <div class="mt-auto pt-2 flex flex-col">
        ...
        <p :class="['text-base font-bold', agotado ? 'text-muted' : 'text-acento-1']">
